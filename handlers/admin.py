...
async def admin_matching(callback: CallbackQuery):
    """Меню мэтчинга"""
    if not is_admin(callback.from_user.id):
        await callback.answer("Нет доступа")
        return
...
async def admin_run_matching(callback: CallbackQuery, bot: Bot):
    ...
    if matches_count > 0:
        ...
        await callback.message.edit_text(
            "❌ Не удалось создать пары по совпадениям\n"
            "Попробуйте принудительный мэтчинг",
            reply_markup=get_admin_matching_inline()
        )
    await callback.answer()
...
async def process_schedule_date(callback: CallbackQuery):
    ...
    if match_id > 0:
        await callback.message.edit_text(
            ...
            f"Пользователи получат уведомления в указанную дату.",
            reply_markup=get_admin_scheduler_inline()
        )
    else:
        await callback.message.edit_text(
...
async def admin_debug(callback: CallbackQuery):
    ...
    if len(active_users) >= 2:
        ...
        debug_info += f"   Общие интересы: {
            ', '.join(common) if common else 'Нет'
            }\n"
